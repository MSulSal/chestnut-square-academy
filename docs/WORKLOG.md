...
- feat(nav-structure): replaced primary first-tab destination with `LIFE AT CHESTNUT`, moved `About Us` to second position, and kept CMYK-style tab color ordering aligned to tab index.
- feat(page-scope): replaced programs-route usage with a dedicated `life-at-chestnut` gallery template and remapped legacy `/our-curriculum` + `/academies/programs` links to `/life-at-chestnut/` in small-business mode.
- feat(home-trim): removed residual Home section blocks containing "Learning with momentum" in the simplification pass while preserving current visual theme/style.
- fix(faq-toggle): resolved parity-mode FAQ double-binding conflict (theme + native scripts) and hardened open/closed state detection using computed visibility checks.
- style(nav-logo): increased navbar logo footprint again (desktop + mobile) for stronger header-height coverage while preserving Elementor Site Identity no-code swaps.
